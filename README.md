...
本项目是一个**非训练项目**，用于对多模态数据（文本和图像）进行聚类分析。项目直接加载HuggingFace预训练模型提取文本、视觉与命名实体等多视角特征（feature_extractor.py），构建三阶张量表示。随后执行t-SVD分解并进行自适应选秩(rank_optimizer.py)与低秩重构(tensor_svd.py)。然后使用聚类算法对数据进行分组。
