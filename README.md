...
本项目实现的是多模态内容联合表征的无监督聚类算法，为**非训练项目**，用于对多模态数据（文本和图像）进行聚类分析。项目直接加载HuggingFace预训练模型提取文本、视觉与命名实体等多视角特征（feature_extractor.py），构建三阶张量表示。

随后执行t-SVD分解并进行自适应选秩(rank_optimizer.py)与低秩重构(tensor_svd.py)。然后使用聚类算法对数据进行分组。

输入：多模态帖子，格式见下**数据格式**章节

输出：完成聚类的各事件目录，格式见下**结果保存形式**章节
...
实验数据为初步采集的500+微博事件，[下载链接](https://bhpan.buaa.edu.cn/link/AA51654EE0CB5E442A86871BD66029C7A9)
### 多模态数据集格式
...
每个事件目录下的 `images/` 子目录中：
- **图片来源**：从原始微博数据目录中按 `id` 匹配复制而来
- **命名规则**：与源数据中的文件名保持一致（如 `{id}.jpg`、`{id}-0.jpg`、`{id}-1.jpg` 等）
